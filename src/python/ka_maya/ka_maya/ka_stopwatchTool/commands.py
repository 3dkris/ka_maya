...
def doStopwatchRecording(rootDir, startFrame=None, endFrame=None):
    ...
    for i, line in enumerate(trace_raw_file):
        ...
        # if line is a End -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -  -
            ...
            metric, node, data = unfinishedLine.split(' ')
            if metric == 'compute':
                if node not in redundancyCounterData:
                    redundancyCounterData[node] = {data:0}

                elif data not in redundancyCounterData[node]:
                    redundancyCounterData[node][data] = 0

                else:
                    redundancyCounterData[node][data] += 1
    ...
    # write redundancyCounterData to file ---------------
    timeData_nodeNames_file = open(timeData_nodeNames, 'r')

    OOOOOOO = "redundancyCounterData.keys()"; print '%s= ' % OOOOOOO, eval(OOOOOOO), ' #', type(eval(OOOOOOO))
    timeData_redundancies_file = open(timeData_redundancies, 'w')
    for iA, lineA in enumerate(timeData_nodeNames_file):
        nodeName = lineA[:-1]

        count = 0
        if nodeName in redundancyCounterData:
            for attrPath in redundancyCounterData[nodeName]:
                count += redundancyCounterData[nodeName][attrPath]

        timeData_redundancies_file.write('%s\n' % str(count))

    redundancyCounterData = None
    timeData_redundancies_file.close()
    timeData_nodeNames_file.seek(0,0)
    timeData_nodeNames_file.close()
